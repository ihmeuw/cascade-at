...
from cascade.input_data.db.csmr import (
   load_csmr_to_t3
)


def test_load_csmr_to_t3_did_upload(mock_execution_context, 
    ...
    mock_upload_csmr_data = mocker.patch(
            "cascade.input_data.db._upload_csmr_data_to_tier_3")

    assert load_csmr_to_t3(execution_context)

    cursor = mock_database_access["cursor"]
    model_version_id = mock_execution_context.parameters.model_version_id

    mock_upload_csmr.assert_called_with(cursor, model_version_id, 
            mock_get_csmr_data)


def test_load_csmr_to_t3_no_upload(mock_execution_context, 
        mock_database_access, mocker):
