...
@pytest.fixture
def time_wt(y0, y1):
    return np.asarray([(2011-y0)/(2012-2011), (y1-2011)/(2012-2011)])


@pytest.fixture
def age_wt(a0, a1):
    return np.asarray([(90-a0)/(90-85), 1, (a1-95)/(125-95)])


def test_covariate_interpolation_over_time(covariate_interpolator, y0, y1, test_cov, test_pop, test_data, time_wt):
    data = test_data.copy()
...
def test_covariate_interpolation_over_age_and_time(covariate_interpolator, y0, y1, a0, a1, test_cov, test_pop,
                                                   test_data, age_wt, time_wt):
    data = test_data.copy()
    data[['age_lower', 'age_upper']] = a0, a1
    data[['time_lower', 'time_upper']] = y0, y1
    cov = test_cov.copy()
