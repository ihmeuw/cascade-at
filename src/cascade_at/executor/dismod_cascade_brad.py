...
if 0:
    ...
    if 10000000000:
        covariate_csv_file_dict = {
        'LDI_pc_log' :
            'ihme_db/DisMod_AT/covariates/GMA_c_LDI_pc_log.csv',
        'mean_BMI' :
            'ihme_db/DisMod_AT/covariates/GMA_c_mean_BMI.csv',
        }
...
mulcov_list_dict = [
    {   # alpha_iota_obesity
        # 3/4/22 'covariate': 'obesity',
        # 3/15/22 
        'covariate': 'LDI_pc_log',
        # 'covariate': 'log_ldi',
        'effected':  'iota',
        'smooth':    'alpha_smooth',
    },
    {   # alpha_chi_log_ldi
        # 3/4/22 'covariate': 'log_ldi',
        # 3/15/22 
        'covariate': 'mean_BMI',
        # 'covariate': 'obesity',
        # gma 'effected':  'chi',
...
def setup_function():
    if not use_csv_files:
        ...
        def switch_to_Brads_naming():
            ...
            # covariate = pd.read_sql('covariate', root_engine)
            ...
            #     write_table_sql(root_engine, 'covariate', covariate,
            #                     dtypes = {'covariate_id': 'integer primary key',
            #                               'covariate_name': 'text',
            #                               'reference': 'real',
            #                               'max_difference': 'real'})
            db.covariate = covariate

            node = pd.read_sql('node', root_engine)
            ...
            write_table_sql(root_engine, 'node', node,
                            dtypes = {'node_id': 'integer primary key',
	                              'node_name': 'text',
	                              'parent': 'integer',
        ...
        def write_node_split_table(
            results_dir, node_split_name_set, root_node_database
        ) :
            if use_csv_files:
                #
                # node_split_table_file
                node_split_table_file = at_cascade.ihme.csv_file['node_split']
                node_split_table_file = f'{results_dir}/{node_split_table_file}'
...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) <= 1:
            ...
            print ('db.nslist', db.nslist, db.nslist_pair)
            print ('db.avgint', db.avgint)
            for cmd in _cmds_:
                tmp = f'{_mvid_}.py {cmd}'
                print (tmp)
                sys.argv = tmp.split()
