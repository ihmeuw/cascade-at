...
class DismodFile:
    ...
    def refresh(self, evict_tables=None):
        ...
        LOG.debug(f"Evicted from wrapper: {evicted}")
    ...
    def _is_dirty(self, table_name):
        ...
        try:
            ...
            if "mutable" in str(te):
                LOG.warning(f"table {table_name} dtypes {table.dtypes}")
    ...
    def flush(self):
        ...
        with self.engine.begin() as connection:
            LOG.debug(f"DismodFile has table data for {', '.join(sorted(self._table_data.keys()))}")
            for table_name in _ordered_by_foreign_key_dependency(Base.metadata, self._table_data.keys()):
                if self._is_dirty(table_name):
                    ...
                    LOG.debug(f"{table_name} did not need to be written")
        ...
        with self.engine.begin() as connection:
            for table_name, table_definition in self._table_definitions.items():
                ...
                if table_info:
                    ...
                    LOG.debug(f"Table {table_name} {table_info}")
