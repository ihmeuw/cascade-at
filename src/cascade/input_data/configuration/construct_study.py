...
def _normalize_covariate_data(bundle_index, study_covariates, id_to_name):
    """
...
def add_special_study_covariates(covariate_records, bundle, average_integrand_cases):
    ...
    covariate_records.id_to_reference["sex"] = 0.0
    ...
    covariate_records.average_integrand_cases = covariate_records.average_integrand_cases(
        one=pd.Series(np.ones(average_integrand_cases.shape[0], dtype=np.double),
                      index=average_integrand_cases.index))
    covariate_records.id_to_reference["one"] = 0.0
    covariate_records.id_to_name[1604] = "one"
    # Would add delta here.
...
def get_bundle_study_covariates(bundle_index, bundle_id, execution_context, tier):
    ...
    records.measurements = _normalize_covariate_data(bundle_index, covariate_data, id_to_name)
