...
class Session:
    ...
    def _fit(self, fit_level, model, data, initial_guess):
        ...
        dm_out, dm_err = run_dismod(self._objects, ["fit", fit_level])
    ...
    def setup_model_for_fit(self, model, data=None, initial_guess=None):
        ...
        """
        prepare_init(
            self._objects, self._locations, self._parent_location, model, data)
        self._objects.set_option(**self._options)
        if self._age_extents:
            self._objects.age_extents = self._age_extents
        run_dismod(self._objects, "init")
        if model.scale_set_by_user:
            scale = model.scale
        else:
            scale = None
        prepare_fit(self._objects, initial_guess, scale)
    ...
    def predict(self, var, avgint, parent_location, weights=None, covariates=None):
        ...
        prepare_predict(self._objects, avgint, var)
        run_dismod(self._objects, ["predict", "truth_var"])
    ...
    def sample(self, simulate_result):
        ...
        """
        run_dismod(self._objects, ["sample", "simulate", simulate_result.count])
