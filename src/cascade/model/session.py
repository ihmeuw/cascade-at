...
class Session:
    ...
    def _fit(self, fit_level, model, data, initial_guess):
        ...
        if initial_guess is not None:
            ...
            self._objects.set_var("start", initial_guess)
        # else use the one generated by the call to init, coming from the mean.
        self._run_dismod(["fit", fit_level])
        return FitResult(self._objects, self._objects.get_var("fit"))
    ...
    def _setup_model_for_fit(self, model, data, initial_guess):
        ...
        self._objects.write_data(data)
        self._run_dismod(["init"])
        if model.scale_set_by_user:
            self._objects.set_var("scale", model.scale)
        elif initial_guess is not None:
            self._objects.set_var("scale", initial_guess)
        else:
            ...
            model._scale = self._objects.get_var("scale")
    ...
    def predict(self, var, avgint, parent_location, weights=None, covariates=None):
        ...
        self._objects.write_model(model, extremal)
        self._objects.write_avgint(avgint)

        self._run_dismod(["init"])
        self._objects.set_var("truth", var)
        self._run_dismod(["predict", "truth_var"])
        predicted, not_predicted = self._objects.get_predict()
    ...
    def simulate(self, model, data, fit_var, simulate_count):
        ...
        self._objects.set_var("truth", fit_var)
    ...
    def sample(self, simulate_result):
        ...
        return self._objects.read_samples()
...
class FitResult:
    ...
    def prior_residuals(self):
        ...
        return self._file_objects.get_prior_residuals()
    ...
    def data_residuals(self):
        ...
        return self._file_objects.get_data_residuals()
